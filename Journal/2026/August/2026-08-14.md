...
```text
The PowerShell script exits with code 1 inside the container. That becomes the container process exit code; `docker run` then exits with that same code, and the host PowerShell session records `docker.exe`'s exit status in `$LASTEXITCODE`. This is useful in CI because the container can act as a standard validation command whose nonzero exit status automatically fails the job.
```
### Reward
- XP: 20
- Stats:
